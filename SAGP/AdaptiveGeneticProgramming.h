...
public:
  AdaptiveGeneticProgramming(int treeDepth, string typeTask)
      : treeDepth(treeDepth), typeTask(typeTask) {
  ...
  void startTrain(double **x, int ammInputs, int amOutPuts, int size,
                  int numIndividuals, int numGeneration);
  Tree getBest() { return bestIndividual; }
    ...
    for (int i = 0; i < size; i++) {
      ...
      for (int j = 1; j < ammOutputs; j++) // Ищем индекс максимального значения
        if (res[j] > res[maxIndex])
          maxIndex = j;
      if ((maxIndex + 1) ==
          x[i][ammInputs]) { // Если индекс совпадает с номером класса, то
                             // увеличиваем счетчик
        count++;
      }
      delete[] res;
  ...
  void numFile(int num) { numberFile = num; }
